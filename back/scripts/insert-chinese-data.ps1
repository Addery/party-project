...
# 将SQL语句保存到UTF-8格式的临时文件
$tempFile = New-TemporaryFile | Rename-Item -NewName { $_ -replace '\.tmp$', '.sql' } -PassThru
Set-Content -Path $tempFile.FullName -Value $sql -Encoding UTF8

# 执行SQL脚本
Get-Content -Path $tempFile.FullName -Encoding UTF8 | mysql --default-character-set=utf8mb4 -h $mysqlHost -u $mysqlUser -p$mysqlPassword -D $mysqlDatabase
