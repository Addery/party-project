...
**参数说明**
- username: 用户名（必填）
- password: 密码（必填，需符合密码强度要求：至少8位，包含大小写字母、数字和特殊字符）
- name: 姓名（必填）
- gender: 性别（可选，默认：男）
- age: 年龄（可选）
- partyStatus: 党员身份（可选，默认：入党申请人）
- branch: 所属支部（可选，默认：默认支部）
- phone: 手机号码（可选）
- email: 邮箱（可选）
- joinDate: 入党日期（可选）
- position: 职务（可选）
...
- 响应示例（密码强度不足）:
  ...
  {
    "code": 500,
    "message": "密码强度不足，需至少8位，包含大小写字母、数字和特殊字符",
    "data": null
  }
  ```
