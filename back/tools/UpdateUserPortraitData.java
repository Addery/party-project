...
@EnableJpaRepositories(basePackages = "com.addery.back.repository")
public class UpdateUserPortraitData {
    public static void main(String[] args) {
        ApplicationContext context = SpringApplication.run(UpdateUserPortraitData.class, args);
        UserPortraitRepository repository = context.getBean(UserPortraitRepository.class);

        // 更新用户ID为2的画像（zhangsan）
        updateUserPortrait(repository, 2, 
        ...
        // 更新用户ID为3的画像（lisi）
        updateUserPortrait(repository, 3, 
        ...
        // 更新用户ID为5的画像（newuser）
        updateUserPortrait(repository, 5, 
        ...
        // 更新用户ID为6的画像（Addery）
        updateUserPortrait(repository, 6, 
        ...
        // 更新用户ID为7的画像（test）
        updateUserPortrait(repository, 7, 
        ...
        updateUserPortrait(repository, 8, 
        ...
        // 更新用户ID为9的画像
        updateUserPortrait(repository, 9, 
        ...
        // 更新用户ID为10的画像（admin）
        updateUserPortrait(repository, 10, 
    ...
    private static void updateUserPortrait(UserPortraitRepository repository, int userId, String learningTags, String learningInterests, String learningPreferences) {
        Optional<UserPortrait> optionalPortrait = repository.findByUserId(userId);
        if (optionalPortrait.isPresent()) {
            UserPortrait portrait = optionalPortrait.get();
            portrait.setLearningTags(learningTags);
            portrait.setLearningInterests(learningInterests);
            portrait.setLearningPreferences(learningPreferences);
